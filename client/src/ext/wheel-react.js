// Customized slightly from
// https://github.com/leon-good-life/wheel-react
// Which had some hard defaults
...
WheelReact.config = function (config) {
  ...
  try {
      for (var _iterator = options[Symbol.iterator](), _step;
...
WheelReact.events = {
    onWheel: function onWheel(e) {
        ...
        if (Math.abs(e.deltaX) > Math.abs(e.deltaY)) {
            if (e.deltaX > 0
                && WheelReact._config.hasOwnProperty('left')) {
                WheelReact._config.left();
            }
            else if (e.deltaX < 0
                     && WheelReact._config.hasOwnProperty('right')) {
