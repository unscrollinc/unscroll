...
const frames = {
    ...
    century: {
        ...
        getColumnLink: (i, offset) => {
            const start = i.start.plus({ years: offset * 10 }).setZone(null);
            const end = start.plus({ years: 9 }).endOf('year');
            const interval = Interval.fromDateTimes(start, end);
            const span = toSpan(interval);
            const title = start.year + 's';
            return { span: span, interval: interval, title: title };
        },
    ...
    decade: {
        ...
        //        format: dt => {
        //            return Math.floor(DateTime.fromISO(dt).year / 10) * 10 + 's';
        //        },
    ...
    day: {
        ...
        elOffset: dt => {
            return Math.ceil(Math.random() * 24, 10);
        },
