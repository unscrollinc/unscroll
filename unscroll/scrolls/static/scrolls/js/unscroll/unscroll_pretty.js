...
(function($, Cookies, MediumEditor) {
  ...
  const d = function(className, kids) {
    return elMaker("div", className, kids);
  };
  const s = function(className, kids) {
    return elMaker("span", className, kids);
  };
  const th = function(className, kids) {
    return elMaker("th", className, kids);
  };
  const tr = function(className, kids) {
    return elMaker("tr", className, kids);
  };
  const td = function(className, kids) {
    return elMaker("td", className, kids);
  };
  ...
  var User = function() {
    ...
    this.initializeDOM = function() {
      ...
      var miniEditize = function(o) {
        ...
        return d("user-form-field", [rubyEl, _el]);
      };
      ...
      if (_user.data.key) {
        ...
        var login = s("login").text("Login").on("click", function(ev) {
          var submit = miniEditize({
            field: "submit",
            type: "submit",
            ruby: undefined,
            data: data
    ...
    this.login = function(data, loginBox, errorsBox) {
      ...
      $.post({
        ...
        error: function(e) {
          if (e.status === 400) {
            this.errorsBox.text(
              "[ERROR] " + e.responseJSON.non_field_errors[0]
            );
    ...
    this.loginDOM = function() {
      var username = s("username").text("You are: " + _user.data.username);
      var notebook = s("notebook").text("Notebook").on("click", function(ev) {
        _user.currentNotebook = new Notebook(undefined, _user);
      });

      var logout = s("logout").text("Logout").on("click", function(ev) {
        _user.logout();
      });
  ...
  var TimeFrame = function(timeline) {
    ...
    this.makePeriod = function(text, start, end) {
      var link = $("<a></a>", {
        class: "period nav",
        href: "/?start=" + start.format() + "&before=" + end.format()
      })
        .html(text)
        .click(function(e) {
          e.preventDefault();
  ...
  var MinutesTimeFrame = function(timeline) {
    ...
    this.getPeriod = function(start) {
      return _timeframe.makePeriod(
        start.format("MMM D, YYYY h:mm:ss") +
          " - " +
          moment(start)
            .add(9, "minutes")
            .add("59", "seconds")
    ...
    this.getTarget = function(start, pointerInteger, pointerMantissa) {
      ...
      var target = pointerFocus
        .clone()
        .add(Math.floor(pointerMantissa * columns), "hours");
      return {
        columns: columns,
        target: target
      };
  ...
  var DaysTimeFrame = function(timeline) {
    ...
    this.getOffset = function(datetime, resolution) {
      if (
        _timeframe.resolutions[resolution] <= _timeframe.resolutions["days"]
      ) {
    ...
    this.getTarget = function(start, pointerInteger, pointerMantissa) {
      ...
      var target = pointerFocus
        .clone()
        .add(Math.floor(pointerMantissa * columns), "days");
      return {
        columns: columns,
        target: target
      };
  ...
  var MonthsTimeFrame = function(timeline) {
    ...
    this.getOffset = function(datetime, resolution) {
      if (
        _timeframe.resolutions[resolution] <= _timeframe.resolutions["months"]
      ) {
  ...
  var YearsTimeFrame = function(timeline) {
    ...
    this.getPeriod = function(start) {
      ...
      return _timeframe.makePeriod(start.year(), from, to);
    };
    this.getOffset = function(datetime, resolution) {
    ...
    this.getTarget = function(start, pointerInteger, pointerMantissa) {
      ...
      var target = pointerFocus
        .clone()
        .add(Math.floor(pointerMantissa * columns), "months");
      return {
        columns: columns,
        target: target
      };
  ...
  var DecadesTimeFrame = function(timeline) {
    ...
    this.getPeriod = function(start) {
      ...
      return _timeframe.makePeriod(
        start.year() + "-" + moment(start).add(9, "years").year(),
    ...
    this.getTarget = function(start, pointerInteger, pointerMantissa) {
      ...
      var target = pointerFocus
        .clone()
        .add(10 * Math.floor(pointerMantissa * columns), "years");
      return {
        columns: columns,
        target: target
      };
  ...
  var CenturiesTimeFrame = function(timeline) {
    ...
    this.getOffset = function(datetime, resolution) {
      return Math.floor(
        (datetime.year() - 100 * Math.floor(datetime.year() / 100)) / 10
      );
    };
    ...
    this.getTarget = function(start, pointerInteger, pointerMantissa) {
      ...
      var target = pointerFocus
        .clone()
        .add(100 * Math.floor(pointerMantissa * columns), "years");
      return {
        columns: columns,
        target: target
      };
  ...
  var MillenniaTimeFrame = function(timeline) {
    ...
    this.getOffset = function(datetime, resolution) {
      return Math.floor(
        (datetime.year() - 1000 * Math.floor(datetime.year() / 1000)) / 100
      );
    };
    this.getEventWidth = function(width) {
      return 4;
    ...
    this.getTarget = function(start, pointerInteger, pointerMantissa) {
      ...
      var target = pointerFocus
        .clone()
        .add(1000 * Math.floor(pointerMantissa * columns), "years");
      return {
        columns: columns,
        target: target
      };
  ...
  var Panel = function(offset, el, timeline) {
    ...
    this.makeColumnsHTML = function() {
      ...
      for (var i = 0; i < _panel.columns; i++) {
        var first = i == 0 ? " first" : "";
        var columnData = _panel.timeline.timeframe.columnStepper(
          i,
          _panel.start
        );
        var column = d("column nav" + first)
          .css({
            width: _panel.columnWidth + "%",
    ...
    this.initializeDOM = function() {
      ...
      var tableburger = "&#9783;";
      var flipper = false;
      var list = d("as-list").html(timelineburger).on("click", function(ev) {
        var t = $(this);
        if (flipper) {
    ...
    this.makeLocation = function() {
      var url =
        "/?start=" + _panel.start.format() + "&before=" + _panel.end.format();
      if (
        _panel.timeline.user.currentScroll &&
        _panel.timeline.user.currentScroll.uuid
      ) {
        url = url + "&scroll=" + _panel.timeline.user.currentScroll.uuid;
      }

      window.history.replaceState({}, "Unscroll: From X to Y", url);
    };
    ...
    this.makeUrl = function() {
      ...
      if (_panel.timeline.search.term) {
        url = url + "&q=" + _panel.timeline.search.term;
      } else if (
        _panel.timeline.user.currentScroll &&
        _panel.timeline.user.currentScroll.uuid
      ) {
      ...
      if (_panel.response) {
        ...
        for (var i = 0; i < es.length; i++) {
          ...
          var el = tr("panel-list", [
            ...
            td("panel-title").html(
              a(e.content_url, "panel-title").html(e.title)
            ),
    ...
    this.render = function() {
      ...
      $.each(es, function(i, e) {
        ...
        e.offset = _panel.timeline.timeframe.getOffset(
          e.datetime,
          e.data.resolution
        );
        ...
        var reservation = _panel.makeReservation(
          e.offset,
          0,
          e.width,
          e.height
  ...
  var Event = function(data, panel) {
    ...
    this.render = function() {
      ...
      if (_event.user.username == _d.username) {
        editButton = a("", "button").html("+[Edit]").on("click", _event.editor);
        deleteButton = a("", "button").html("[X]").on("click", _event.delete);
      }
    ...
    this.makeEditor = function(exists) {
      ...
      var newEvent = $("<div></div>", { class: "editable-event" }).append(
        $("<div></div>").append(
          ...
          makeInput(
            ...
            "Date/time",
            false,
            _event.panel.timeline.pos.target.format()
          ),
          makeInput(
            "resolution",
            "Resolution",
      ...
      _el.on("mousemove touchmove", function(e) {
        ...
        if (_timeline.pos.touching) {
          ...
          if (_timeline.pos.lastDragX !== null) {
            _timeline.pos.dragX =
              100 *
              (_timeline.pos.pageX - _timeline.pos.lastDragX) /
              _timeline.window.width;
          } else {
          ...
          _timeline.pos.lastOffset = _timeline.pos.offset;
          var val = {
            marginLeft: _timeline.pos.offset - 100 + "%",
            cursor: "move"
          };
          _el.css(val);
        } else {
        ...
        _timeline.pos.mouseOffset =
          _timeline.pos.timelineOffset +
          _timeline.pos.pageX / _timeline.window.width;
        _timeline.pos.pointerInteger = Math.floor(_timeline.pos.mouseOffset);
        var tmpMantissa = _timeline.pos.mouseOffset % 1;
        _timeline.pos.pointerMantissa = tmpMantissa < 0
          ? 1 + tmpMantissa
          : tmpMantissa;
        ...
        var gt = _timeline.timeframe.getTarget(
          _timeline.start,
          _timeline.pos.pointerInteger,
          _timeline.pos.pointerMantissa
        );
  ...
  var Notebook = function(scroll, user) {
    ...
    var editize = function(o) {
      ...
      if (isRichText) {
        el = d("field-input scroll-" + fieldName + " " + fieldName).html(
          data[fieldName]
        );
        ...
        el = i(
          "field-input scroll-" + fieldName + " " + fieldName,
          data[fieldName]
        );
      ...
      if (isNote) {
        ...
        buttons.append(
          s("button move").text("Move").on("click", function(ev) {
            _notebook.moveStatus.fromNote.el = caller;
            _notebook.setRange();
          }),
          s("button range").text("Range").on("click", function(ev) {
            _notebook.moveStatus.throughNote.el = caller;
            _notebook.setRange();
          }),
          s("button drop").text("Drop").on("click", function(ev) {
            _notebook.moveStatus.toBeforeNote.el = caller;
            _notebook.rearrange();
          }),
          s("button delete").text("Delete").on("click", function(ev) {
    ...
    this.initializeDOM = function() {
      ...
      $("#note-default-create-button").off().on("click", function(ev) {
        ev.preventDefault();
        console.log("Clicked on default create button");
        _notebook.makeItem("default", undefined, undefined);
      });

      $("#notebook-create-button").on("click", function(ev) {
        _notebook.create();
      });

      $("#insert-space").off().on("click", function(ev) {
        ev.preventDefault();
        _notebook.makeItem("spacer");
      });

      $("#insert-image").off().on("click", function(ev) {
        ev.preventDefault();
    ...
    this.loadScroll = function() {
      var start = _notebook.data.first_event
        ? moment(_notebook.data.first_event)
        : undefined;
      var before = _notebook.data.last_event
        ? moment(_notebook.data.last_event)
        : undefined;
      _notebook.read();
    ...
    this.noteScanner = function() {
      var toCreate = $.grep(_notebook.items, function(e) {
        return e.needsCreated;
      });
  ...
  var NotebookList = function(user) {
    ...
    this.initialize = function() {
      _notebooklist.scrolls = $.map(user.data.full_scrolls, function(s) {
        return new NotebookListItem(s, _notebooklist);
      });
      var els = $.map(_notebooklist.scrolls, function(s) {
        return s.el;
      });
    ...
    this.initializeDOM = function(scrolls) {
      _notebooklist.el.append(
        $("<table></table>").append(
          tr("notebook-list-item header").append(
  ...
  var NotebookListItem = function(item, notebooklist, user) {
    ...
    this.initializeDOM = function() {
      var line = tr("notebook-list-item").append(
        td("notebook-list-date").text(
          moment(_notebooklistitem.data.created).fromNow()
        ),
        ...
        td("notebook-list-public").text(
          _notebooklistitem.data.public ? "Public" : "Private"
        )
      );
      line.on("click", function(ev) {
        var notebook = new Notebook(
          _notebooklistitem.data,
          _notebooklistitem.user
        );
